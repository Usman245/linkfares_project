...
import { hotelBook } from "../../redux/features/hotelBooking";
import { useDispatch, useSelector } from "react-redux";

// Tabs ki data define ki gayi hai
const tabs = [
...
const CityHotelCards = () => {
  const [selected, setSelected] = useState("Karachi");
  const dispatch = useDispatch();
  const { data = [], loading, error } = useSelector((state) => state.bookingHotel);
      ...
      <div className="py-20 flex flex-col gap-5">
        <div className="flex flex-col justify-center gap-2 items-center">
          <p className="text-3xl font-semibold text-black font-poppins">
            Hotels in your <span className="text-green-600">home </span>
          </p>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-3">
            {filteredHotels.length > 0 ? (
              filteredHotels.map((hotel, index) => (
                <CityCards key={index} hotel={hotel} />
              ))
            ) : (
